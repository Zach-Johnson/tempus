...
export const useExercisesStore = defineStore("exercises", () => {
  ...
  const exercisesWithDerivedCategories = computed(() => {
    return exercises.value.map((exercise) => {
      ...
      const derivedCategoryIds = exercise.tagIds && exercise.tagIds.length > 0
  ...
  async function fetchExercise(id) {
    ...
    try {
      ...
      error.value = err.message || `Failed to fetch exercise with ID ${id}`;
  ...
  async function updateExercise(id, exerciseData, updateMask) {
    ...
    try {
      ...
      error.value = err.message || `Failed to update exercise with ID ${id}`;
  ...
  async function deleteExercise(id) {
    ...
    try {
      ...
      error.value = err.message || `Failed to delete exercise with ID ${id}`;
  ...
  async function deleteExerciseImage(imageId) {
    ...
    try {
      ...
      exercises.value.forEach((exercise) => {
        if (exercise.images) {
          exercise.images = exercise.images.filter((img) => img.id !== imageId);
        }
      });

      // Remove image from currentExercise if applicable
      if (currentExercise.value && currentExercise.value.images) {
        currentExercise.value.images = currentExercise.value.images.filter(
          (img) => img.id !== imageId,
        );
      }
    } catch (err) {
      error.value = err.message || `Failed to delete image with ID ${imageId}`;
  ...
  async function addExerciseLink(exerciseId, linkData) {
    ...
    try {
      ...
      if (currentExercise.value && currentExercise.value.id === exerciseId) {
  ...
  async function deleteExerciseLink(linkId) {
    ...
    try {
      ...
      if (currentExercise.value && currentExercise.value.links) {
        currentExercise.value.links = currentExercise.value.links.filter(
          (link) => link.id !== linkId,
        );
      }
    } catch (err) {
      error.value = err.message || `Failed to delete link with ID ${linkId}`;
